...
export class DuelHUD {
  ...
  constructor() {
    this._timerText = new Text({
      text: "99",
      style: { fontFamily: "monospace", fontSize: 36, fill: COL_TEXT, fontWeight: "bold" },
    });
    this._p1Name = new Text({
      text: "",
      style: { fontFamily: "monospace", fontSize: 18, fill: COL_TEXT },
    });
    ...
    this._comboText = new Text({
      text: "",
      style: { fontFamily: "monospace", fontSize: 28, fill: COL_COMBO, fontWeight: "bold" },
    });
    this._announcementText = new Text({
      text: "",
      style: { fontFamily: "monospace", fontSize: 64, fill: COL_TEXT, fontWeight: "bold",
  ...
  build(sw: number, sh: number, p1Name: string, p2Name: string): void {
    this._screenW = sw;
    // HP bars fill from edge to near center, leaving space for timer
    this._hpBarWidth = Math.round((sw / 2) - TIMER_SIZE - HP_BAR_MARGIN * 2);
    this._p1Name.text = p1Name;
    this._p2Name.text = p2Name;

    // Position names
    this._p1Name.position.set(HP_BAR_MARGIN, HP_BAR_Y - 22);
    this._p2Name.position.set(sw - HP_BAR_MARGIN - this._hpBarWidth, HP_BAR_Y - 22);

    // Timer background
    this._timerBg.clear();
    const timerX = (sw - TIMER_SIZE) / 2;
    this._timerBg.roundRect(timerX, HP_BAR_Y - 8, TIMER_SIZE, TIMER_SIZE, 8);
    this._timerBg.fill({ color: COL_TIMER_BG, alpha: 0.85 });
    this._timerBg.stroke({ color: COL_HP_BORDER, width: 2 });

    this._timerText.position.set(sw / 2 - 18, HP_BAR_Y + 2);

    // Announcement centered
    this._announcementText.anchor.set(0.5);
    this._announcementText.position.set(sw / 2, sh * 0.35);
  ...
  update(state: DuelState): void {
    ...
    this._p1Bar.clear();
    this._p1Bar.rect(HP_BAR_MARGIN, HP_BAR_Y, this._hpBarWidth, HP_BAR_HEIGHT);
    ...
    this._p1Bar.rect(HP_BAR_MARGIN, HP_BAR_Y, this._hpBarWidth * p1Pct, HP_BAR_HEIGHT);
    ...
    this._p2Bar.clear();
    const p2X = sw - HP_BAR_MARGIN - this._hpBarWidth;
    this._p2Bar.rect(p2X, HP_BAR_Y, this._hpBarWidth, HP_BAR_HEIGHT);
    ...
    const p2FillW = this._hpBarWidth * p2Pct;
    this._p2Bar.rect(p2X + this._hpBarWidth - p2FillW, HP_BAR_Y, p2FillW, HP_BAR_HEIGHT);
    ...
    this._drawRoundDots(this._p2Rounds, sw - HP_BAR_MARGIN - this._hpBarWidth, HP_BAR_Y + HP_BAR_HEIGHT + 6, state, 1);
    ...
    if (activeCombo && activeCombo.comboCount > 1) {
      ...
      this._comboText.position.set(
        activeCombo.position.x - 40,
        activeCombo.position.y - 220,
      );
    } else {
      this._comboText.visible = false;
    }
