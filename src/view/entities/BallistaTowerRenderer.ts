// Ballista tower renderer - enhanced tower with ballista for extended range
...
const COL_METAL_DK = 0x666666; // Dark metal for bolts

// Animation timing
const FLAG_SPEED = 3.2;

// ---------------------------------------------------------------------------
// BallistaTowerRenderer
// ---------------------------------------------------------------------------

export class BallistaTowerRenderer {
  readonly container = new Container();

  private _base = new Graphics(); // tower body, static details
  private _flag = new Graphics(); // waving flag at the top
  private _ballista = new Graphics(); // ballista weapon system
  ...
  constructor(owner: string | null) {
    ...
    this.container.addChild(this._ballista);
  ...
  private _drawBallista(): void {
    const g = this._ballista;
    g.clear();

    // Ballista base platform
    g.fill({ color: COL_WOOD });
    g.rect(16, 18, 32, 8);

    // Ballista frame sides
    g.fill({ color: COL_WOOD_DK });
    g.rect(18, 12, 4, 14);
    g.rect(42, 12, 4, 14);

    // Ballista arms (wooden beams)
    g.fill({ color: COL_WOOD });
    g.rect(12, 8, 4, 4);
    g.rect(48, 8, 4, 4);

    // Ballista mechanism (metal parts)
    g.fill({ color: COL_METAL });
    g.rect(22, 14, 20, 4);
    g.circle(32, 16, 3);

    // Ballista bow arms
    g.stroke({ color: COL_WOOD_DK, width: 3 });
    g.beginPath();
    g.moveTo(8, 10);
    g.quadraticCurveTo(32, 0, 56, 10);
    g.stroke();

    // Ballista string
    g.stroke({ color: COL_METAL_DK, width: 1 });
    g.beginPath();
    g.moveTo(8, 10);
    g.lineTo(56, 10);
    g.stroke();
