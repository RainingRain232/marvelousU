...
export class MageTowerRenderer {
    ...
    private _drawBase(): void {
        ...
        // Step lines
        g.moveTo(10, TH - 7).lineTo(TW - 10, TH - 7)
        ...
        this._drawBrickPattern(g, bodyL + 2, bodyTop + 2, bodyR - bodyL - 4, bodyBot - bodyTop - 4);

        // ── Weathering / moss patches ──
        g.ellipse(bodyL + 8, bodyBot - 8, 5, 2).fill({ color: COL_MOSS, alpha: 0.4 });
        g.ellipse(bodyR - 6, bodyBot - 12, 4, 2).fill({ color: COL_MOSS, alpha: 0.35 });
        ...
        for (let by = ironGateY + 5; by < bodyBot - 3; by += 7) {
            g.moveTo(ironGateX + 1, by).lineTo(ironGateX + ironGateW - 1, by)
                .stroke({ color: COL_IRON, width: 1, alpha: 0.5 });
        }
        // Ring handle
        g.circle(ironGateX + ironGateW - 4, ironGateY + ironGateH * 0.5, 1.5)
    ...
    private _drawButtress(g: Graphics, x: number, botY: number, dir: number): void {
        ...
        // Base pedestal
        g.rect(x + (dir > 0 ? 0 : -12), botY - 4, 12, 4)
            .fill({ color: COL_BASALT_DK });
    }
    ...
    private _drawPortalStatic(g: Graphics, cx: number): void {
        ...
        // Dark interior
        g.rect(cx - portalW / 2, portalY + 8, portalW, portalH - 8)
            .fill({ color: 0x0a0a1e });
        g.ellipse(cx, portalY + 8, portalW / 2, 8)
            .fill({ color: 0x0a0a1e });
    ...
    private _drawLancetWindow(g: Graphics, x: number, y: number, w: number, h: number): void {
        // Dark frame
        g.rect(x - 1, y + 4, w + 2, h - 4)
        ...
        g.rect(x, y + 5, w, h - 5)
            .fill({ color: 0x221144, alpha: 0.8 });

        // Star dots (constellation)
        const stars = [
            { dx: 0.2, dy: 0.25 }, { dx: 0.7, dy: 0.2 },
            { dx: 0.5, dy: 0.5 }, { dx: 0.3, dy: 0.7 },
            { dx: 0.8, dy: 0.65 }, { dx: 0.6, dy: 0.85 },
        ];
        for (const s of stars) {
            g.circle(x + w * s.dx, y + 5 + (h - 5) * s.dy, 0.8)
                .fill({ color: 0xffffcc, alpha: 0.8 });
        }
        ...
        // Lead frame lines
        g.moveTo(x + w / 2, y + 2).lineTo(x + w / 2, y + h)
    ...
    private _drawRune(g: Graphics, x: number, y: number, alpha: number): void {
        ...
        g.moveTo(x, y - 3).lineTo(x, y + 3)
            .stroke({ color: COL_RUNE, width: 0.5, alpha: alpha * 0.7 });
        g.moveTo(x - 2, y).lineTo(x + 2, y)
            .stroke({ color: COL_RUNE, width: 0.5, alpha: alpha * 0.7 });
    }

    private _drawRuneCircle(g: Graphics, x: number, y: number, r: number, alpha: number): void {
        g.circle(x, y, r).stroke({ color: COL_RUNE, width: 1, alpha });
        g.circle(x, y, r * 0.5).stroke({ color: COL_RUNE, width: 0.5, alpha: alpha * 0.6 });
        // Radiating dots
        for (let i = 0; i < 6; i++) {
            const a = (i / 6) * Math.PI * 2;
            g.circle(x + Math.cos(a) * (r + 2), y + Math.sin(a) * (r + 2), 0.8)
                .fill({ color: COL_RUNE, alpha: alpha * 0.5 });
        }
    }
    ...
    private _updateThreads(time: number): void {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const orbitR = 8;
            const ox = cx + Math.cos(a) * orbitR;
            const oy = baseY + Math.sin(a) * 2 + i * 4 + Math.sin(time * 1.2 + i * 1.5) * 2;

            // Thread from chunk to crystal (apex at top)
            g.moveTo(ox, oy)
                .quadraticCurveTo(cx + Math.sin(time + i) * 3, baseY - 14, cx, baseY - 20)
                .stroke({ color: COL_MANA_THREAD, width: 1.2, alpha });

            // Small spark at junction
            g.circle(ox, oy, 1.5 + flicker)
                .fill({ color: COL_MANA_THREAD, alpha: alpha * 0.6 });
        }
    }
    ...
    private _updateCrystal(time: number): void {
        ...
        g.circle(cx, cy, ringR)
            .stroke({ color: COL_GOLD, width: 1.2, alpha: 0.7 });
        g.circle(cx, cy, ringR * 0.65)
        ...
        // Inner glow core
        g.circle(cx, cy, crR * 0.4)
            .fill({ color: COL_CRYSTAL_CORE, alpha: 0.5 + pulse * 0.3 });

        // Radial glow halo
        g.circle(cx, cy, ringR + 3)
            .stroke({ color: COL_CRYSTAL, width: 0.8, alpha: 0.15 + pulse * 0.1 });
    }
    ...
    private _updateGarden(time: number): void {
        ...
        for (let i = 0; i < plantCount; i++) {
            ...
            g.circle(px + sway, terY - stemH, 2)
                .fill({ color: COL_MANA_PLANT, alpha: 0.6 + glow * 0.4 });
            // Bright inner core
            g.circle(px + sway, terY - stemH, 0.8)
                .fill({ color: 0xffffff, alpha: 0.3 + glow * 0.3 });
        }
    }
    ...
    private _updateOwl(time: number): void {
        ...
        // Ear tufts
        g.moveTo(-3, -9).lineTo(-4, -13).lineTo(-1, -10)
            .closePath().fill({ color: COL_OWL });
        g.moveTo(3, -9).lineTo(4, -13).lineTo(1, -10)
            .closePath().fill({ color: COL_OWL });
        ...
        if (isBlinking) {
            // Closed eyes (lines)
            g.moveTo(-3.5, -6).lineTo(-0.5, -6)
                .stroke({ color: 0x000000, width: 0.8 });
            g.moveTo(0.5, -6).lineTo(3.5, -6)
                .stroke({ color: 0x000000, width: 0.8 });
        } else {
            // Open eyes
        ...
        // Beak
        g.moveTo(-1, -4.5).lineTo(0, -3).lineTo(1, -4.5)
            .closePath().fill({ color: 0xcc8833 });
    ...
    private _updateClouds(time: number): void {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            g.ellipse(cloudX, cloudY, size, size * 0.5)
                .fill({ color: COL_CLOUD, alpha });
        }
        ...
        for (let i = 0; i < 3; i++) {
            ...
            g.ellipse(cloudX, cloudY, 4, 2)
                .fill({ color: COL_CLOUD_LT, alpha });
        }
    }
    ...
    private _updatePortal(time: number): void {
        ...
        g.rect(cx - portalW / 2 + 1, portalY + 2, portalW - 2, portalH - 2)
            .fill({ color: COL_PORTAL_BLUE, alpha: alpha * 0.4 });
        g.ellipse(cx, portalY + 2, portalW / 2 - 1, 6)
            .fill({ color: COL_PORTAL_BLUE, alpha: alpha * 0.4 });
