...
export class TownRenderer {
    ...
    private _drawHouse(g: Graphics, x: number, y: number, w: number, h: number, isChurch = false): void {
        // Walls
        g.rect(x, y, w, h).fill({ color: COL_STUCCO }).stroke({ color: 0xaaaaaa, width: 1 });

        // Roof
        if (isChurch) {
            g.moveTo(x - 4, y + 2).lineTo(x + w / 2, y - 15).lineTo(x + w + 4, y + 2).closePath().fill({ color: 0x444444 });
        } else {
            g.moveTo(x - 2, y + 2).lineTo(x + w / 2, y - 8).lineTo(x + w + 2, y + 2).closePath().fill({ color: COL_ROOF });
        }
    ...
    private _drawBellTower(g: Graphics, x: number, y: number): void {
        const w = 15, h = 50;
        g.rect(x, y, w, h).fill({ color: COL_STONE }).stroke({ color: COL_STONE_DK, width: 1.5 });
        // Domed top
        g.moveTo(x - 2, y + 2).bezierCurveTo(x + w / 2, y - 15, x + w / 2, y - 15, x + w + 2, y + 2).fill({ color: 0x1a1a2e });
    }

    private _drawGate(g: Graphics, x: number, y: number): void {
        const w = 40, h = 25;
        // Side walls
        g.rect(x - 30, y + 5, 30, 15).fill({ color: COL_STONE }).stroke({ color: COL_STONE_DK, width: 2 });
        g.rect(x + w, y + 5, 30, 15).fill({ color: COL_STONE }).stroke({ color: COL_STONE_DK, width: 2 });

        // Archway
        g.rect(x, y, w, h).fill({ color: COL_STONE }).stroke({ color: COL_STONE_DK, width: 2.5 });
        g.ellipse(x + w / 2, y + h, w / 2 - 5, h - 5).fill({ color: 0x000000 });
    ...
    private _drawSmallTower(g: Graphics, x: number, y: number): void {
        g.rect(x, y, 15, 30).fill({ color: COL_STONE }).stroke({ color: COL_STONE_DK, width: 2 });
    }
