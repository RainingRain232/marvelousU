// Procedural sprite generator for the Minor Fire Elemental unit type.
//
// 96×96 px per frame (2×2 tiles). A smaller, less imposing fire elemental:
//   • Cracked dark stone body with glowing lava fissures
//   • Smaller flame crown on head
//   • Compact stone limbs with ember glow
//   • No weapon — fights with fists
//   • Fewer embers, simpler details than T5 version
...
function drawShadow(g: Graphics, cx: number, cy: number, scale = 1): void {
  ...
  g.ellipse(cx, cy + 2, 16 * scale, 4 * scale)
    .fill({ color: COL_LAVA, alpha: 0.06 });
}
...
function drawLegs(
  ...
  for (const side of [-1, 1] as const) {
    ...
    g.ellipse(legX, kneeY + offset * 0.4, 2.5, 2)
      .fill({ color: COL_LAVA_BRIGHT, alpha: 0.6 });
...
function drawTorso(
  ...
  g.roundRect(cx - 14, torsoY - 8, 28, 16, 3)
    .fill({ color: COL_OBSIDIAN });
  g.roundRect(cx - 14, torsoY - 8, 28, 16, 3)
    .stroke({ color: COL_OBSIDIAN_LT, width: 0.7 });
  ...
  g.moveTo(cx - 1, torsoY - 8)
    .quadraticCurveTo(cx + 2, torsoY, cx - 1, torsoY + 6)
    .stroke({ color: COL_MAGMA_CORE, width: 0.7 });
  ...
  g.ellipse(cx, torsoY, 4 + breathe * 0.3, 5 + breathe * 0.3)
    .fill({ color: COL_MAGMA_CORE, alpha: 0.12 + breathe * 0.02 });
  ...
  g.roundRect(cx - 16, torsoY + 7, 32, 5, 2)
    .fill({ color: COL_OBSIDIAN });
  g.ellipse(cx, torsoY + 9, 2.5, 2).fill({ color: COL_LAVA_BRIGHT });
}
...
function drawHead(
  ...
  g.roundRect(cx - 9, headY - 6, 18, 14, 4)
    .fill({ color: COL_OBSIDIAN });
  g.roundRect(cx - 9, headY - 6, 18, 14, 4)
    .stroke({ color: COL_OBSIDIAN_LT, width: 0.5 });
  ...
  // flame crown (smaller, fewer tongues)
  const flames = [
    { dx: 0, h: 12, w: 5 },
    { dx: -5, h: 9, w: 4 },
    { dx: 5, h: 10, w: 4 },
  ...
  for (const f of flames) {
    const fx = cx + f.dx;
    const flicker = Math.sin(flamePhase * 4 + f.dx * 0.3) * 2;
    ...
    g.moveTo(fx - f.w * 0.5, baseY2)
      ...
      .closePath()
      .fill({ color: COL_FLAME_OUTER, alpha: 0.65 });
...
function drawArm(
  ...
  g.moveTo(shoulderX, shoulderY).lineTo(elbowX, elbowY)
    .stroke({ color: COL_OBSIDIAN, width: 6 });
  g.moveTo(shoulderX, shoulderY).lineTo(elbowX, elbowY)
    .stroke({ color: COL_LAVA, width: 1.2 });

  g.circle(elbowX, elbowY, 4).fill({ color: COL_OBSIDIAN });
  g.circle(elbowX, elbowY, 2).fill({ color: COL_LAVA_DK, alpha: 0.6 });
  ...
  g.moveTo(elbowX, elbowY).lineTo(handX, handY)
    .stroke({ color: COL_OBSIDIAN, width: 5 });
  g.moveTo(elbowX, elbowY).lineTo(handX, handY)
    .stroke({ color: COL_LAVA, width: 1 });

  // fist
  g.circle(handX, handY, 4).fill({ color: COL_BASALT });
  g.circle(handX, handY, 4).stroke({ color: COL_OBSIDIAN, width: 0.7 });
}
...
function drawEmbers(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (py > baseY - 50 && py < baseY + 4) {
      const col =
        i % 3 === 0 ? COL_MAGMA_CORE : i % 3 === 1 ? COL_EMBER : COL_FLAME_MID;
...
function generateIdleFrame(g: Graphics, frame: number): void {
  ...
  drawEmbers(g, CX, GY, flamePhase * 0.3, 8, 20, 0.4);
}
...
function generateMoveFrame(g: Graphics, frame: number): void {
  ...
  drawEmbers(g, CX + 6, GY, flamePhase * 0.4, 10, 22, 0.5);
}
...
function generateAttackFrame(g: Graphics, frame: number): void {
  ...
  drawEmbers(g, attackCX, GY, flamePhase * 0.3, 12, 22, 0.6 + impactFlash * 0.3);
}
...
function generateCastFrame(g: Graphics, frame: number): void {
  ...
  if (auraProgress > 0) {
    for (let i = 0; i < 5; i++) {
      ...
      g.moveTo(startX, GY + 2).lineTo(endX, endY)
        .stroke({ color: COL_LAVA, width: 1.5 * auraProgress });
    }
  }
  ...
  drawEmbers(g, CX, GY, flamePhase * 0.3, 16 + auraProgress * 8, 30, 0.5 + auraProgress * 0.3);

  // fire ring around caster
  if (auraProgress > 0.3) {
    const vortexAlpha = (auraProgress - 0.3) * 0.4;
    for (let i = 0; i < 6; i++) {
      const angle = i * 1.05 + flamePhase * 2;
...
function generateDieFrame(g: Graphics, frame: number): void {
  ...
  if (crumble < 0.8) {
    ...
    g.roundRect(CX + tilt - 18, torsoY - 11, 36, 22, 3)
      .fill({ color: cool > 0.5 ? COL_COOLED : COL_BASALT });
    if (cool < 0.7) {
      g.moveTo(CX + tilt, torsoY - 8)
        .lineTo(CX + tilt + 2, torsoY)
        .lineTo(CX + tilt - 1, torsoY + 6)
        .stroke({ color: COL_LAVA, width: 1.5, alpha: 1 - cool * 1.3 });
    }
      ...
      for (let i = 0; i < cnt; i++) {
        ...
        if (h > 1) {
          g.moveTo(CX + tilt + dx - 1.5, headY - 6)
            ...
            .closePath()
            .fill({ color: COL_FLAME_OUTER, alpha: 0.4 * (1 - cool * 2) });
        }
      }
    }
  }
  ...
  if (t > 0.2) {
    for (let i = 0; i < 3; i++) {
      ...
      g.circle(sx, sy, 3 + i).fill({ color: COL_SMOKE, alpha: 0.12 * (1 - t) });
    }
  }

  drawEmbers(g, CX, GY, flamePhase, Math.max(0, 8 - Math.floor(t * 10)), 18, 0.3 * (1 - t));
}
...
export function generateMinorFireElementalFrames(
  renderer: Renderer,
