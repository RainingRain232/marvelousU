// Procedural sprite generator for the Ancient Archer unit type.
//
// A T4 "Ancient One" variant of the Archer — bigger (48×72 frame),
// blackened/corroded armor over chainmail, grey undead-looking skin,
// ancient darkwood bow with spectral bowstring.
...
const COL_BOW = 0x2a1a10;
const COL_BOW_DK = 0x1a0e08;
const COL_STRING = 0x8888aa;
...
function drawTorso(g: Graphics, cx: number, top: number, h: number, tilt = 0): void {
  ...
  // Lighter breastplate — archer wears less heavy plate
  ...
  // Shoulder guards (lighter than axeman)
  g.ellipse(x - 1, top + 3, 5, 4)
    .fill({ color: COL_PLATE })
    .stroke({ color: COL_PLATE_DK, width: 0.5 });
  g.circle(x - 1, top + 2, 0.8).fill({ color: COL_RIVET });
  g.ellipse(x + tw + 1, top + 3, 5, 4)
    .fill({ color: COL_PLATE })
    .stroke({ color: COL_PLATE_DK, width: 0.5 });
...
function drawQuiver(g: Graphics, cx: number, cy: number): void {
  ...
  // Arrow fletching
  g.rect(rx + 1, ry - 5, 1.5, 5).fill({ color: COL_ARROW });
  g.rect(rx + 3.5, ry - 5, 1.5, 5).fill({ color: COL_ARROW });
}
...
function drawBow(g: Graphics, bowX: number, bowY: number, angle: number, pull: number): void {
  ...
  // Bow limbs — dark ancient wood
  g.moveTo(x1, y1)
    .quadraticCurveTo(cpx, cpy, x2, y2)
    .stroke({ color: COL_BOW, width: 2.5 });
  g.moveTo(x1, y1)
    .quadraticCurveTo(cpx - 1, cpy, x2, y2)
    .stroke({ color: COL_BOW_DK, width: 0.6, alpha: 0.5 });

  // Spectral bowstring
  const stringX = -pull * 12;
  const sx = bowX + (stringX * cos);
  const sy = bowY + (stringX * sin);
  g.moveTo(x1, y1)
    .lineTo(sx, sy)
    .lineTo(x2, y2)
    .stroke({ color: COL_STRING, width: 0.8, alpha: 0.7 });
...
function generateIdleFrame(g: Graphics, frame: number): void {
  ...
  // Bow arm (left) — holding bow at side
  drawArm(g, CX - 8, torsoTop + 5, CX - 10, torsoTop + 10);
  drawBow(g, CX - 11, torsoTop + 8, -0.2, 0);
...
function generateMoveFrame(g: Graphics, frame: number): void {
  ...
  // Bow swings while walking
  const bowX = CX - 10 + walk;
  const bowY = torsoTop + 8 + bob;
...
function generateAttackFrame(g: Graphics, frame: number): void {
  ...
  const stringPullX = bowX - pull * 12;
  ...
  if (frame === 6) {
    g.moveTo(bowX + 5, bowY)
      .lineTo(bowX + 35, bowY)
      .stroke({ color: 0xaaaacc, width: 1.8, alpha: 0.5 });
  }
}
...
function generateCastFrame(g: Graphics, frame: number): void {
  ...
  drawBow(g, CX + 10, torsoTop - 6 - raise, Math.PI / 4, 0);
...
export function generateAncientArcherFrames(
  renderer: Renderer,
