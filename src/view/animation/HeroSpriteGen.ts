...
export class HeroSpriteGen {
  ...
  private static _drawHero(
    ...
    if (walking) {
      // Walking pose
      g.rect(centerX - 6, centerY + 8, 4, 12 + legOffset).fill({ color: legColor });
      g.rect(centerX + 2, centerY + 8, 4, 12 - legOffset).fill({ color: legColor });
    } else {
      // Standing pose
    ...
    g.rect(centerX - 8, centerY - 8 + bodyBob, 16, 16).fill({ color: bodyColor });
    g.rect(centerX - 8, centerY - 8 + bodyBob, 16, 16).stroke({ color: 0x808080, width: 1 });
    ...
    if (attacking) {
      ...
      // Shield arm
      g.rect(centerX - 12, centerY - 4 + bodyBob, 4, 8).fill({ color: armColor });
      ...
      // Normal pose
      g.rect(centerX - 12, centerY - 4 + bodyBob, 4, 8).fill({ color: armColor });
      g.rect(centerX + 8, centerY - 4 + bodyBob, 4, 8).fill({ color: armColor });
      ...
      // Sword
      g.rect(centerX + 10, centerY - 8 + bodyBob, 2, 16).fill({ color: 0xE5E5E5 });
      g.rect(centerX + 10, centerY - 8 + bodyBob, 2, 16).stroke({ color: 0x808080, width: 1 });
    }
    
    // Head with helmet
    const helmetColor = 0xC0C0C0; // Silver helmet
    g.circle(centerX, centerY - 12 + bodyBob, 6).fill({ color: helmetColor });
    g.circle(centerX, centerY - 12 + bodyBob, 6).stroke({ color: 0x808080, width: 1 });
    
    // Helmet plume
    const plumeColor = 0xFF4500; // Orange-red plume
    g.beginPath();
    g.moveTo(centerX, centerY - 18 + bodyBob);
    g.quadraticCurveTo(centerX + 3, centerY - 22 + bodyBob, centerX + 2, centerY - 26 + bodyBob);
    g.quadraticCurveTo(centerX, centerY - 24 + bodyBob, centerX - 2, centerY - 26 + bodyBob);
    g.quadraticCurveTo(centerX - 3, centerY - 22 + bodyBob, centerX, centerY - 18 + bodyBob);
    ...
    if (powerGlow > 0) {
      ...
      g.circle(centerX, centerY, glowSize).fill({ color: 0xFFD700, alpha: glowAlpha });
      g.circle(centerX, centerY, glowSize).stroke({ color: 0xFFA500, alpha: glowAlpha, width: 2 });
    }
  }
}
