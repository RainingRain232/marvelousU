...
export class DistortionFX {
    ...
    init(vm: ViewManager): void {
        ...
        EventBus.on("projectileCreated", ({ projectileId, origin, target }) => {
            ...
            if (projectileId.startsWith("dist-")) {
    ...
    private _emitTrailBurst(cx: number, cy: number): void {
        for (let i = 0; i < TRAIL_PER_BURST; i++) {
            ...
            const tint = DISTORTION_TINTS[Math.floor(Math.random() * DISTORTION_TINTS.length)];
            this._pool.emit({
    ...
    private _onTeleport(from: { x: number; y: number }, to: { x: number; y: number }): void {
