// Race selection screen — placeholder for future race/faction content.
// Currently shows only "Coming Soon" and a button to proceed to the Armory.
...
export class RaceSelectScreen {
  ...
  init(vm: ViewManager): void {
    ...
    // Title
    const title = new Text({ text: "SELECT YOUR RACE", style: STYLE_TITLE });
    ...
    const iconText = new Text({ text: "?", style: new TextStyle({
      fontFamily: "monospace", fontSize: 42, fill: 0x334455, fontWeight: "bold",
    })});
    ...
    // Description
    const desc = new Text({
      text: "Race selection will allow you to choose a faction with unique units, abilities, and buildings. Check back in a future update.",
      style: STYLE_SUB,
    });
    ...
    nextBtn.position.set(CW - 156, CH - 46);
