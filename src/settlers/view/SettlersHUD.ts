  ...
  private _updateMinimap(state: SettlersState): void {
    ...
    for (let tz = 0; tz < map.height; tz++) {
      for (let tx = 0; tx < map.width; tx++) {
        ...
        // Resource deposit icons (colored dots)
    ...
    for (let tz = 0; tz < map.height; tz++) {
      for (let tx = 0; tx < map.width; tx++) {
        ...
        if (owner !== 1) continue; // only enemy territory (player 1)

        const px = tx * sx;
        const pz = tz * sz;

        // Check each edge - draw border if neighbor is not enemy territory
        // Top edge
    ...
    // Draw soldiers as colored dots (green=friendly, red=enemy)
    ...
    // Draw flags (smaller, less prominent)
