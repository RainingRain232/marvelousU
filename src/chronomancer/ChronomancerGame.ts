// ---------------------------------------------------------------------------
// Chronomancer — Main Game Orchestrator
...
import type { CMState } from "./types";
import { createCMState, loadCMMeta, saveCMMeta, awardCMShards } from "./state/ChronomancerState";
import { CM } from "./config/ChronomancerBalance";
import {
  updatePlayer, tryShoot, tryDash, tryPulse, tryChronoShift, tryTimeFreeze,
  startCharge, releaseCharge,
  updateEnemies, updateProjectiles, updateTimeZones, updateTemporalEchoes,
  updateBoss, checkBossProjectileHits,
  updateWave, updateTimers, updateParticles, updateFloatTexts,
  updateShockwaves, updatePickups, updateHazards, spawnDeathEffect,
} from "./systems/ChronomancerSystem";
import { ChronomancerRenderer } from "./view/ChronomancerRenderer";

export class ChronomancerGame {
  private _state!: CMState;
  private _renderer = new ChronomancerRenderer();
  private _tickerCb: ((ticker: Ticker) => void) | null = null;
  private _meta = loadCMMeta();
  ...
  async boot(): Promise<void> {
    ...
    this._meta = loadCMMeta();
    this._state = createCMState(sw, sh, this._meta);
  ...
  private _initInput(): void {
    this._keyDownHandler = (e: KeyboardEvent) => {
      ...
      if (s.phase === CMPhase.DEAD) {
        ...
        else if (e.code === "Digit3") { this._buyUpgrade("dashCooldown", 3); e.preventDefault(); }
        else if (e.code === "Digit4") { this._buyUpgrade("pulsePower", 3); e.preventDefault(); }
        else if (e.code === "Digit5") { this._buyUpgrade("chronoShift", 2); e.preventDefault(); }
        return;
      }

      // Pause toggle
      if (e.code === "Escape") {
        s.phase = s.phase === CMPhase.PLAYING ? CMPhase.PAUSED : CMPhase.PLAYING;
        e.preventDefault();
        return;
      }

      if (s.phase !== CMPhase.PLAYING) return;
      ...
      // Chrono Shift on Q
      if (e.code === "KeyQ") { tryChronoShift(s); e.preventDefault(); }
      // Time Freeze on E
      if (e.code === "KeyE") { tryTimeFreeze(s); e.preventDefault(); }
    };
    ...
    // Left click: instant fire + track hold for auto-fire; right click: start charging
    this._clickHandler = (e: PointerEvent) => {
      const s = this._state;
      if (!s || s.phase !== CMPhase.PLAYING) return;
      if (e.button === 0) {
    ...
    // Release: stop auto-fire or release charged bolt
  ...
  private _loop(rawDt: number): void {
    ...
    if (s.phase === CMPhase.PLAYING) {
      ...
      // Auto-fire while left mouse held (and not charging right-click)
      if (this._leftMouseDown && !s.chargingBolt) {
        tryShoot(s);
      }
      const diedFromEnemies = updateEnemies(s, dt);
      const diedFromProj = updateProjectiles(s, dt);
      checkBossProjectileHits(s);
      const diedFromBoss = updateBoss(s, dt);
      const diedFromHazards = updateHazards(s, dt);
      updateTimeZones(s, dt);
      updateTemporalEchoes(s, dt);
      updateShockwaves(s, dt);
      updatePickups(s, dt);
      updateWave(s, dt);
      updateTimers(s, dt);
    ...
    updateFloatTexts(s, dt);
    updateShockwaves(s, dt);
    if (s.phase !== CMPhase.PLAYING) s.time += dt;
    this._renderer.render(s, sw, sh, this._meta);
  }
  ...
  private _startGame(): void {
    ...
    this._meta = loadCMMeta();
    this._state = createCMState(sw, sh, this._meta);
    this._state.phase = CMPhase.PLAYING;
  }

  private _die(): void {
    const s = this._state;
    if (s.phase === CMPhase.DEAD) return;
    s.phase = CMPhase.DEAD;
    spawnDeathEffect(s);
    const meta = loadCMMeta();
    meta.gamesPlayed++;
    const score = Math.floor(s.score);
    if (score > meta.highScore) meta.highScore = score;
    if (s.wave > meta.bestWave) meta.bestWave = s.wave;
    awardCMShards(meta, score);
    saveCMMeta(meta); this._meta = meta;
  }
  ...
  private _buyUpgrade(key: string, maxLevel: number): void {
    ...
    const costs = (CM as Record<string, unknown>).UPGRADE_COSTS as Record<string, number[]>;
  ...
  private _exit(): void { window.dispatchEvent(new Event("chronomancerExit")); }
}
